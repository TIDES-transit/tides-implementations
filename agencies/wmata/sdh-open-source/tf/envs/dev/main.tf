...
module "lakekeeper" {
  ...
  # flow; group membership alone is not sufficient)
...
module "dagster" {
  ...
  trino_app_service_principal_object_id      = var.trino_app_registration_object_id
...
module "opa" {
  ...
  # Use Lakekeeper's credentials for now
...
module "echo_http" {
  source                               = "../../modules/debug/apps/echo_http"
  resource_group_name                  = var.resource_group_name
  datahub_container_app_environment_id = module.common.container_app_environment_id
}
...
# deployments.
module "hello_world" {
  source                                  = "../../modules/debug/apps/hello_world"
  arm_subscription_id                     = var.arm_subscription_id
  resource_group_name                     = var.resource_group_name
  resource_group_location                 = var.resource_group_location
  datahub_container_app_environment_id    = module.common.container_app_environment_id
  datahub_container_registry_id           = module.common.container_registry_id
  datahub_container_registry_login_server = module.common.container_registry_login_server
  workload_identity_resource_id           = module.dagster.dagster_workload_identity_id
  has_entra                               = var.has_entra
}
